  ...
  const fetchOne = async (id) => {
    ...
    const p = (async () => {
      ...
      if (!r.ok) {
        const body = await safeBody(r);
        if (r.status === 404) {
          // Negative caching: store missing trips so post-CRL retired IDs don't hammer upstream
          const nullRes = { data: null };
          cache.set(id, { data: nullRes, ts: Date.now() });
          return null;
        }
        const err = new Error(`Upstream error: ${r.status}`); err.status = r.status; err.body = body; throw err;
      }
...
function ok(res, payload, ttlSeconds) {
  res.setHeader("Cache-Control", `public, max-age=0, s-maxage=${Math.max(1, ttlSeconds)}, stale-while-revalidate=10`);
